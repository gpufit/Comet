...
from pair_indices import pair_indices_simplified, pair_indices_lex, pair_indices_lex_dblchk, pair_indices,\
    pair_indices_lex_floor


@cuda.jit
def cost_function_full_3d(d_locs_time, d_idx_i, d_idx_j, d_sigma, d_sigma_factor, d_val, d_deri=np.array([[]]),
    ...
    if pos < d_idx_i.size:
        d_val[pos] = (math.exp(1)**(-(((d_locs_coords[d_idx_i[pos], 0] - mu[d_locs_time[d_idx_i[pos]], 0]) -
                                      ...
                                      ((d_locs_coords[d_idx_i[pos], 2] - mu[d_locs_time[d_idx_i[pos]], 2]) -
                                       (d_locs_coords[d_idx_j[pos], 2] - mu[d_locs_time[d_idx_j[pos]], 2]))**2) /
                                    ((d_sigma*d_sigma_factor) ** 2)))
        cuda.atomic.add(d_deri, (d_locs_time[d_idx_i[pos]], 0), d_val[pos] * 2.*(d_locs_coords[d_idx_i[pos], 0] -
                                                                                 d_locs_coords[d_idx_j[pos], 0] +
                                                                                 mu[d_locs_time[d_idx_j[pos]], 0] -
                                                                                 mu[d_locs_time[d_idx_i[pos]], 0]) /
                        (d_sigma*d_sigma_factor) ** 2)
        ...
        cuda.atomic.add(d_deri, (d_locs_time[d_idx_i[pos]], 2), d_val[pos] * 2. * (
                    d_locs_coords[d_idx_i[pos], 2] - d_locs_coords[d_idx_j[pos], 2] + mu[d_locs_time[d_idx_j[pos]], 2] -
                    mu[d_locs_time[d_idx_i[pos]], 2]) / (d_sigma * d_sigma_factor) ** 2)
...
def cuda_wrapper(mu, d_locs_coords, d_locs_time, d_idx_i, d_idx_j, d_sigma, d_sigma_factor, d_val, d_deri):
    ...
    cost_function_full_3d[blockspergrid, threadsperblock](d_locs_time, d_idx_i, d_idx_j, d_sigma, d_sigma_factor, d_val,
                                                          d_deri, d_locs_coords, mu)
    #cuda.synchronize()
    deri = d_deri.copy_to_host()
    d_deri[:] = 0
    return -np.nansum(d_val), -deri.flatten()[3:]


def optimize_3d(n_segments, locs_nm, sigma_nm=30, drift_max_nm=150, sigma_factor=1, threshold_estimator_nm=5,
                display_steps=False):
    ...
    idx_i, idx_j = pair_indices_lex(locs_nm[:, :3].copy(), drift_max_nm)
    # idx_i, idx_j = pair_indices_lex_dblchk(locs_nm[:, :3].copy(), drift_max_nm)
    # idx_i, idx_j = pair_indices(locs_nm[:, :3].copy(), drift_max_nm)

    d_locs_coords = cuda.to_device(np.asarray(locs_nm[:, :3], dtype=np.float64).copy())
    d_locs_time = cuda.to_device(np.asarray(locs_nm[:, 3], dtype=np.int64).copy())
    ...
    while optimization_done == 0:
        ...
        result = minimize(cuda_wrapper, drift_estimate[3:], method='L-BFGS-B', options={'disp': display_steps},
                          bounds=bounds, jac=True,
                          args=(d_locs_coords, d_locs_time, d_idx_i, d_idx_j, d_sigma, d_sigma_factor, d_val, d_deri))
        if np.mean(np.abs(result['x']-drift_estimate[3:])) < threshold_estimator_nm and result['success']:
            optimization_done = 1
...
def cuda_wrapper_heavy_chunked(mu, d_locs_coords, d_locs_time, d_idx_i, idx_i, d_idx_j, idx_j, d_sigma, d_sigma_factor, d_val, d_deri,
                         chunk_size):
    # currently broken
    val = 0
    d_val_sum = cuda.to_device(np.zeros(1, dtype=np.float64))
    ...
    mu = np.append([0, 0, 0], np.asarray(mu))
    mu = np.asarray(mu.reshape(int(mu.size / 3), 3), dtype=np.float64)
    n_chunks = int(np.ceil(len(idx_i) / chunk_size))
    for i in range(n_chunks-1):
        d_idx_i[:] = idx_i[i * int(chunk_size):(i + 1) * int(chunk_size)]
        d_idx_j[:] = idx_j[i * int(chunk_size):(i + 1) * int(chunk_size)]
        idc = np.arange(chunk_size) + i * chunk_size
        blockspergrid = (chunk_size + (threadsperblock - 1)) // threadsperblock
        cost_function_full_3d_chunked[blockspergrid, threadsperblock](d_locs_time, idc[0], chunk_size, d_idx_i, d_idx_j,
                                                                      d_sigma,
                                                                      d_sigma_factor, d_val, d_val_sum, d_deri,
                                                                      d_locs_coords, mu)
        #cuda.synchronize()
        val += d_val_sum.copy_to_host()
    n_remaining = d_idx_i.size - (n_chunks - 1) * chunk_size
    idc = np.arange(n_remaining) + (n_chunks - 1) * chunk_size
    d_idx_i = idx_i[(n_chunks-1) * int(chunk_size):]
    d_idx_j = idx_j[(n_chunks-1) * int(chunk_size):]
    threadsperblock = 256
    blockspergrid = (n_remaining + (threadsperblock - 1)) // threadsperblock
    cost_function_full_3d_chunked[blockspergrid, threadsperblock](d_locs_time, idc[0], n_remaining, d_idx_i, d_idx_j,
                                                                  d_sigma,
                                                                  d_sigma_factor, d_val, d_val_sum, d_deri,
                                                                  d_locs_coords, mu)
    val += d_val_sum.copy_to_host()
    deri = d_deri.copy_to_host()
    d_deri[:] = 0
    return -np.nansum(val), -deri.flatten()[3:]


def optimize_3d_heavy_chunked(n_segments, locs_nm, sigma_nm=30, drift_max_nm=300, sigma_factor=1, threshold_estimator_nm=5,
                display_steps=False):
    # currently broken
    drift_estimate = np.zeros(3 * n_segments)
    bounds = []
    ...
    d_idx_j[:] = idx_j
    d_sigma = np.float64(sigma_nm)
    val = np.zeros_like(idx_i, np.float64)
    d_val = cuda.to_device(np.zeros(int(chunk_size)))
    deri = np.zeros((n_segments, 3), dtype=np.float64)
    d_deri = cuda.to_device(deri)
    ...
    while optimization_done == 0:
        d_sigma_factor = np.float64(sigma_factor)
        result = minimize(cuda_wrapper_heavy_chunked, drift_estimate[3:], method='L-BFGS-B', options={'disp': display_steps},
                          bounds=bounds, jac=True,
                          args=(d_locs_coords, d_locs_time, d_idx_i, idx_i, d_idx_j, idx_j, d_sigma, d_sigma_factor, d_val,d_deri,
                                chunk_size))
        if np.mean(np.abs(result['x']-drift_estimate[3:])) < threshold_estimator_nm and result['success']:
            optimization_done = 1
        elif not result['success']:
